...
def train_step(sess, train_op, global_step, train_step_kwargs,accurancies):
  ...
  if 'should_log' in train_step_kwargs:
    if sess.run(train_step_kwargs['should_log']):
      ...
      accuranc = float(sess.run(accurancies))
      logging.info('training_error = %.4f', accuranc)
      
      lossfile = open('lossfile.txt','a')
      lossfile.write(str(accuranc)+',')
      lossfile.close()	
        ...
        try:
          while not sv.should_stop():
            total_loss, should_stop = train_step_fn(
                sess, train_op, global_step, train_step_kwargs,accurancies)
            #print ('hier2')
            #print (total_loss)
